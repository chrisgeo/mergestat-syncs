...
async def build_person_summary_response(
    *,
    ...
    async with clickhouse_client(db_url) as client:
        ...
        identity_coverage_pct = float(coverage_sources) / 2.0 * 100.0
    ...
    *,
    ...
    async with clickhouse_client(db_url) as client:
        ...
        timeseries = [
            MetricTimeseriesPoint(
                day=row["day"],
                value=transform(float(row.get("value") or 0.0)),
            )
        ...
        for key in breakdowns.keys():
            ...
            breakdowns[key] = [
                MetricBreakdownItem(
                    label=str(row.get("label") or ""),
                    value=detail["transform"](float(row.get("value") or 0.0)),
                )
                for row in rows
            ]
    ...
    for row in rows:
        items.append(
            PullRequestRow(
                ...
                review_latency_hours=row.get("review_latency_hours"),
                link=None,
            )
        )
